...
def taskSquare(draw):
	...
	nn1 = nn.NeuralNetwork(2, 0.001, 100, 50)
...
def taskSemiCircle(draw):
	...
	nn1 = nn.NeuralNetwork(2, 0.0008, 25, 50)
...
def taskMnist():
	...
	nn1 = nn.NeuralNetwork(10, 0.001, 30, 20)
	nn1.addLayer(FullyConnectedLayer(28*28, 10, 'softmax'))	
	###############################################
	nn1.train(XTrain, YTrain, XVal, YVal, False, True)
	pred, acc  = nn1.validate(XTest, YTest)
	print('Test Accuracy ',acc)
...
def taskCifar10():	
	...
	# nn1 = nn.NeuralNetwork(10, 0.001, 64, 40)
	# nn1.addLayer(ConvolutionLayer((3, 32, 32), (11, 11), 4, 7, 'relu'))
	# nn1.addLayer(FlattenLayer())
	# nn1.addLayer(FullyConnectedLayer(64, 10, 'softmax'))
	nn1 = nn.NeuralNetwork(10, 0.0009, 64, 20)
	nn1.addLayer(ConvolutionLayer((3, 32, 32), (10, 10), 4, 11, 'relu'))
	nn1.addLayer(FlattenLayer())
	nn1.addLayer(FullyConnectedLayer(36, 30, 'relu'))
	nn1.addLayer(FullyConnectedLayer(30, 10, 'softmax'))
	###################################################
