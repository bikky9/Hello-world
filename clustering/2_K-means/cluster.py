...
def kmeans_iteration_one(data, centroids, distance):
	...
	for i in range(len(data)):
		...
		for j in range(len(centroids)):
			if(distance(centroids[j],data[i]) < minimumdistance):
				minimumdistance = distance(centroids[j],data[i])
				minimumcentroid = j
		labels.append(minimumcentroid)
	for j in range(len(centroids)):
		idx = [i for i, e in enumerate(labels) if e == j]
		if(len(idx)!=0):
			new = data[idx[0]]
			for k in range(1,len(idx)):
				new = [new[i] + data[idx[k]][i] for i in range(len(new))] 
			new_centroids.append(tuple([float(x)/len(idx) for x in new]))
		else:
...
def iteration_many(data, centroids, distance, maxiter, algorithm, epsilon=1e-1):
	...
	for i in range(maxiter):
		...
		if(hasconverged(new_centroids,all_centroids[-1])):
	...
	for i in range(len(data)):
		...
		for j in range(len(centroids)):
			if(distance(centroids[j],data[i]) < minimumdistance):
				minimumdistance = distance(centroids[j],data[i])
				minimumcentroid = j
		sse+=(distance(centroids[minimumcentroid],data[i]))**2
