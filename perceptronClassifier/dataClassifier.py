...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # print(len(datum))
    # first basic feature is to count number of black pixels this would tell the area of the figure
    # Second feature is to find the perimeter of the shape by calculating number of immediate white neighbours for black pixels.
    # Using these two features I am getting 80% accuracy
    features[0] = sum(datum)
    count = 0
    for i in range(1,DATUM_HEIGHT-1):
        for j in range(1,DATUM_WIDTH-1):
            if(getPixelVal(i,j) != 1):
                if (getPixelVal(i,j-1) ==  1) or (getPixelVal(i, j+1) == 1) or  (getPixelVal(i-1, j) == 1) or (getPixelVal(i+1, j) ==1):
                    count += 1
    features[1] = count
