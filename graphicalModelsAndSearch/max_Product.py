            ...
            for i in range(0, N):
                ...
                for j in range(0, N):
                    if(testlist[t] in emission_dict and j != special1 and j != special and i != special1 and i != special):
                        temp.append(SEQSCORE[j, t-1] + transition_matrix[j, i] + emission_dict[testlist[t]][j])
                    elif(j != special1 and j != special and i != special1 and i != special):
                        temp.append(SEQSCORE[j, t-1] + transition_matrix[j, i] + np.log(lbd*1e-8))
                    else:
                        temp.append(-1e10)
                SEQSCORE[i, t] = max(temp)
                BACKPTR[i, t] = temp.index(max(temp))
        C = np.zeros((T))
        for i in range(N):
            SEQSCORE[i, T-1] += transition_matrix[i, special1]
        C[T-1] = np.argmax(SEQSCORE[:, T-1])
        C[T-1] = BACKPTR[int(C[T-1]),T-1]
